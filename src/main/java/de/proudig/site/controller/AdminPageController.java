...
public class AdminPageController {
    ...
    @GetMapping
    public ResponseEntity<List<PageDto>> getPages(@RequestParam(required = false) PageCategory category, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "50") int size) {
        ...
        List<PageDto> dtos = result.getContent().stream().map(p -> PageDto.builder().id(p.getId()).slug(p.getSlug()).title(p.getTitle()).category(p.getCategory()).content(p.getContent()).excerpt(p.getExcerpt()).coverImageId(p.getCoverImage() != null ? p.getCoverImage().getId() : null).tags(p.getTagsList()).metaData(p.getMetaData()).status(p.getStatus()).authorId(p.getAuthor().getId()).authorName(p.getAuthor().getFirstName() + " " + p.getAuthor().getLastName()).publishedAt(p.getPublishedAt()).createdAt(p.getCreatedAt()).updatedAt(p.getUpdatedAt()).build()).collect(Collectors.toList());
    ...
    @PutMapping("/{id}")
    public ResponseEntity<PageDto> updatePage(@PathVariable String id, @RequestBody PageUpdateRequest request) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
