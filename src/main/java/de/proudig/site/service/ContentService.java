...
public class ContentService {
    ...
    public ContentBlockDto getPublishedContent(String sectionKey) {
        ContentBlock block = contentBlockRepository.findBySectionKey(sectionKey).orElseThrow(() -> new NoSuchElementException("Content block not found: " + sectionKey));
    ...
    public List<ContentBlockDto> getAllPublished() {
        return contentBlockRepository.findAllByPublishedAtIsNotNull().stream().map(this::mapToDto).collect(Collectors.toList());
    }
    ...
    public List<ContentBlockDto> getAllForAdmin() {
        return contentBlockRepository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    public ContentBlockDto saveDraft(String sectionKey, String contentJson, User user) {
        ContentBlock block = contentBlockRepository.findBySectionKey(sectionKey).orElseGet(() -> ContentBlock.builder().sectionKey(sectionKey).build());
    ...
    private ContentBlockDto mapToDto(ContentBlock block) {
        return ContentBlockDto.builder().id(block.getId()).sectionKey(block.getSectionKey()).content(block.getContent()).draftContent(block.getDraftContent()).updatedAt(block.getUpdatedAt()).publishedAt(block.getPublishedAt()).build();
    }

    public ContentService(final ContentBlockRepository contentBlockRepository, final ActivityLogService activityLogService) {
