...
public class DocumentShareService {
    ...
    public DocumentShareDto shareDocument(String documentId, String sharedWithEmail, DocumentPermission permission, Instant expiresAt, User sharedBy) {
        ...
        User sharedWithUser = userRepository.findByEmail(sharedWithEmail)
                .orElseThrow(() -> new NoSuchElementException("User not found: " + sharedWithEmail));
        ...
        activityLogService.log(sharedBy, "SHARE", "DOCUMENT", document.getId(),
                "Shared with " + sharedWithUser.getEmail());
        return mapToDto(share);
    }

    public List<DocumentShareDto> getSharedWithMe(User user) {
        return documentShareRepository.findBySharedWith(user)
                .stream()
                .filter(share -> share.getExpiresAt() == null || share.getExpiresAt().isAfter(Instant.now()))
                .map(this::mapToDto)
    ...
    public void removeShare(String shareId, User user) {
        ...
        if (!share.getDocument().getUploadedBy().getId().equals(user.getId()) &&
            !share.getSharedWith().getId().equals(user.getId())) {
            throw new IllegalAccessError("Access denied");
        }

        activityLogService.log(user, "UNSHARE", "DOCUMENT", share.getDocument().getId(),
                "Unshared with " + share.getSharedWith().getEmail());
    ...
    private DocumentShareDto mapToDto(DocumentShare share) {
        return DocumentShareDto.builder()
                ...
                .sharedWithName(share.getSharedWith().getFirstName() + " " + share.getSharedWith().getLastName())
