...
public class PageService {
    ...
    public org.springframework.data.domain.Page<PageDto> getPublishedBlogPosts(Pageable pageable) {
        org.springframework.data.domain.Page<Page> pages = pageRepository.findByCategoryAndStatus(
    ...
    public org.springframework.data.domain.Page<PageDto> getPublishedSeminars(Pageable pageable) {
        org.springframework.data.domain.Page<Page> pages = pageRepository.findByCategoryAndStatus(
                PageCategory.SEMINAR, PageStatus.PUBLISHED, pageable);
        return pages.map(this::mapToDto);
    }

    public org.springframework.data.domain.Page<PageDto> getArchivedSeminars(Pageable pageable) {
        org.springframework.data.domain.Page<Page> pages = pageRepository.findByCategoryAndStatusNot(
                PageCategory.SEMINAR, PageStatus.DRAFT, pageable);
        return pages.map(this::mapToDto);
    }
    ...
    public List<String> getAllTags(PageCategory category) {
        ...
        return rawTags.stream()
                .flatMap(tags -> List.of(tags.split(",\\s*")).stream())
                .distinct()
                .sorted()
    ...
    public PageDto createPage(PageCreateRequest request, User user) {
        ...
        if (request.getCoverImageId() != null) {
            page.setCoverImage(mediaRepository.findById(request.getCoverImageId())
                    .orElseThrow(() -> new NoSuchElementException("Cover image not found")));
        }
    ...
    public void autoArchivePastSeminars() {
        org.springframework.data.domain.Page<Page> seminars = pageRepository.findByCategoryAndStatus(
                PageCategory.SEMINAR, PageStatus.PUBLISHED, Pageable.unpaged());
